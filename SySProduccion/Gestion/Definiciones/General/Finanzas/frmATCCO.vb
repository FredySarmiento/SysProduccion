...
#Region "020 frm Eventos - Principal"
    ...
    Private Sub frmATCCO_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            grl_RemoveHandler()
        Catch ex As Exception
            MsgBox("Error : frmATCCO_Disposed")
            MsgBox(ex.Message)
        End Try
    End Sub


    Private Sub frmFECCO_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            Me.Text += "  -  " + vg_sNomCia
            grl_RemoveHandler()
            grl_AddHandler()
...
#Region "040 BD Procedimientos sobre la BD. Personalizados"
    Private Function BD_CrearActualizar(ByVal wp_sCmdSel As String) As Boolean
        ...
        Try
            Plo_Var_Frm()
            BD_eATCCO_Var()
            wl_bYesNo = clnATCCO.cnCrearActATCCO(wl_eATCCO,
                                                 wr_bEOFE1,
                                                 wp_sCmdSel)
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub BD_DatPrmTipTbl(ByRef wl_sTipTbl As String)
        Try
            wl_sNomTblE1 = "ATCCO"
            wl_sNomCmpE11 = "sCodCco"
            wl_sTipCmpE11 = "C"
            wl_sTitCmpE11 = "Código Ctro Costos "
            wl_sDatCmpE11 = txtsCodCco.Text
    ...
    Private Sub BD_eATCCO_Var()
        ...
        Catch ex As Exception
            MsgBox(ex.Message)
            MsgBox("Error en el SUB: BD_eATCCO_Var")
        End Try
    End Sub
    Private Sub BD_Var_eATCCO()
        Try
            With wl_eATCCO
                vl_sCodCco = .sCodCco
                vl_sNomCco = .sNomCco
        ...
        Catch ex As Exception
            MsgBox(ex.Message)
            MsgBox("Error en el SUB: BD_Var_eATCCO")
        End Try
    End Sub
    Private Sub BD_eATCCO_DT(ByVal wp_DTE1 As DataTable)
        Try
            ...
            moo_eATCCO_DT(wl_eATCCO,
                          wp_DTE1,
                          wl_nRow)
            wl_sDatCmpE12 = txtsCodCco.Text

        Catch ex As Exception
            MsgBox("Error :  BD_eATCCO_DT")
            MsgBox(ex.Message)
        End Try
    End Sub
#End Region
#Region "041 BD Procedimientos sobre la BD. Comunes"
    Private Sub BD_tsbPASU()
        Try
            Plo_ActivarCmpAct()
            txtsCodCco.Enabled = False
            vl_sCodCco = wl_DTE1.Rows(0).Item("sCodCco")
            wl_sDatCmpE11 = vl_sCodCco
            BD_BuscarATCCO()
            BD_Var_eATCCO()
            Plo_Frm_Var()
            mof_ActivarOK(Me)
...
#Region "045 BD Llamar frm"
    Private Sub BD_frmATCCO()
        Try
            Dim wl_frmLst As New frmGQLst
            wl_eATCCO = New moe_Estructuras.eATCCO
            wg_sTblCon1 = "ATCCO"
            wl_frmLst.ShowDialog()
            If wl_frmLst.wp_eATCCO.sCodCco <> "" Then
                wl_eATCCO = wl_frmLst.wp_eATCCO
            End If
        Catch ex As Exception
            MsgBox("Error :  BD_frmATCCO")
            MsgBox(ex.Message)
        End Try
    End Sub
#End Region
...
#Region "CommandButtons_Click ABRECO Controladores de Eventos (Remitente_evento)"
    ...
    Private Sub cmdBuscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdBuscar.Click
        Try
            ...
            wl_bYesNo = False
            mof_LmpMsgError()
            vl_sCodCco = txtsCodCco.Text
            If txtsCodCco.Text <> "" Then
                BD_DatPrmTipTbl(wl_sTipTbl)
                wl_sCmpOrdE11 = ""
                wl_sTipOrdE11 = ""
                wr_bEOFE1 = BD_BuscarATCCO()
                If wr_bEOFE1 Then
                    wp_sNomTbl = wl_sNomTblE1
                    wp_sDatCmp = txtsCodCco.Text
    ...
    Private Sub cmdCrear_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdCrear.Click
        Try
            ...
            If txtsCodCco.Text <> "" Then
                vl_sCodCco = txtsCodCco.Text
                BD_DatPrmTipTbl(wl_sTipTbl)
                wr_bEOFE1 = mof_fConATCCO(vl_sCodCco,
                                          wl_eATCCO)
                If wr_bEOFE1 Then
                    wl_bYesNo = BD_CrearActualizar(wp_sCmdSel)
                    If wl_bYesNo Then
            ...
            If txtsCodCco.Text <> "" Then
                ...
                If wp_nResMsg = 6 Then
                    ...
                    If Not wr_bEOFE1 Then
                        BD_eATCCO_DT(wl_DTE1)
                        wl_bVda = False
                        wp_sNomTbl = ""
                        wl_bYesNo = mov_VdasCodCco(vl_sCodCco,
                                                   wp_sNomTbl,
                                                   wl_bVda)
                        If wl_bVda = True Then
                            wl_bYesNo = clnATCCO.cnEliATCCO(wl_eATCCO,
                                                            wr_bEOF)
                            If wl_bYesNo Then
...
#Region "060 CommandButtons_Click Lista Controladores de Eventos (Remitente_evento)"
    Private Sub cmdLstATCCO_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdLstATCCO.Click
        Try
            Dim wl_frmLst As New frmGQLst
            BD_frmATCCO()
            If wl_eATCCO.sCodCco <> "" Then
                BD_Var_eATCCO()
                Plo_Frm_Var()
                txtsNomCco.Focus()
                mof_ActivarOK(Me)
            ...
            End If
        Catch ex As Exception
            MsgBox("Error en: cmdLstATCCO_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
...
#Region "080 Plo Procedimientos del Formulario. Personalizados"
    ...
    Private Sub Plo_IniciarVar()
        Try
            wl_bYesNo = False
            wl_sCodErrAdi = "10-304"
            wl_sCodErrCon = "10-104"
            wl_sCodErrOk = "20-001"
    ...
    Private Sub Plo_Frm_Var()
        Try
            txtsCodCco.Text = vl_sCodCco
            txtsNomCco.Text = vl_sNomCco
...
#Region "081 Plo Procedimientos del Formulario. Comunes"
    ...
    Private Sub Plo_ActivarCmpAct()
        Try
            txtsCodCco.Enabled = True
            txtsNomCco.Enabled = True
...
#Region "082 Elo Eventos locales del Formulario"
    Private Sub txtsCodCco_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtsCodCco.KeyUp
        Try
...
#Region "110 grl Changed"
    ...
    Private Sub grl_Txt_Changed(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If DirectCast(sender, TextBox).Name = "txtsCodCco" Then
            wl_sClaPpl = txtsCodCco.Text
            mof_LmpMsgError()
        Else
            grl_TextChange()
        End If
    ...
    Private Sub grl_Txt_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        ...
        If cmdCancelar.Focused = False Then
            If wl_cvTxt IsNot Nothing Then
                If wl_cvTxt.Name = "txtsCodCco" Then
                    mov_VdaTxt(wl_bVda,
                               txtsCodCco)
                End If
            End If
