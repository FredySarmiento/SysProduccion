...
#Region "014 Objetos locales"
    Dim clnATZON As New ClaseNegocios.ClnATZON
...
#Region "020 frm Eventos - Principal"
    ...
    Private Sub frmATZON_Disposed(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            grl_RemoveHandler()
        Catch ex As Exception
            MsgBox("Error : frmATZON_Disposed")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub frmATZON_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            Me.Text += "  -  " + vg_sNomCia
            grl_RemoveHandler()
            grl_AddHandler()
...
#Region "040 BD Procedimientos sobre la BD. Personalizados"
    Private Function BD_CrearActualizar(ByVal wp_sCmdSel As String) As Boolean
        ...
        Try
            Plo_Var_Frm()
            BD_eATZON_Var()
            wl_bYesNo = clnATZON.cnCrearActATZON(wl_eATZON,
                                                 wr_bEOFE1,
                                                 wp_sCmdSel)
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub BD_DatPrmTipTbl(ByRef wl_sTipTbl As String)
        Try
            wl_sNomTblE1 = "ATZON"
            wl_sNomCmpE11 = "sCodZon"
            wl_sTipCmpE11 = "C"
            wl_sTitCmpE11 = "Código Zona "
            wl_sDatCmpE11 = txtsCodZon.Text
    ...
    Private Sub BD_eATZON_Var()
        ...
        Catch ex As Exception
            MsgBox(ex.Message)
            MsgBox("Error en el SUB: BD_eATZON_Var")
        End Try
    End Sub
    Private Sub BD_Var_eATZON()
        Try
            With wl_eATZON
                vl_sCodZon = .sCodZon
                vl_sNomZon = .sNomZon
        ...
        Catch ex As Exception
            MsgBox(ex.Message)
            MsgBox("Error en el SUB: BD_Var_eATZON")
        End Try
    End Sub
    Private Sub BD_eATZON_DT(ByVal wp_DTE1 As DataTable)
        Try
            ...
            moo_eATZON_DT(wl_eATZON,
                          wp_DTE1,
                          wl_nRow)
            wl_sDatCmpE12 = txtsCodZon.Text

        Catch ex As Exception
            MsgBox("Error :  BD_eATZON_DT")
            MsgBox(ex.Message)
        End Try
    End Sub
#End Region
#Region "041 BD Procedimientos sobre la BD. Comunes"
    Private Sub BD_tsbPASU()
        Try
            Plo_ActivarCmpAct()
            txtsCodZon.Enabled = False
            vl_sCodZon = wl_DTE1.Rows(0).Item("sCodZon")
...
#Region "042 BD Busquedas"
    Private Function BD_BuscarATZON() As Boolean
        Try
            wr_bEOFE2 = True
            wl_eATZON = New moe_Estructuras.eATZON
            wr_bEOFE2 = mof_fConATZON(vl_sCodZon,
                                      wl_eATZON)
        Catch ex As Exception
            MsgBox(ex.Message)
            MsgBox("Error en el SUB: BD_BuscarATZON")
        End Try
        Return wr_bEOFE2
    End Function
...
#Region "045 BD Llamar frm"
    Private Sub BD_frmATZON()
        Try
            Dim wl_frmLst As New frmGQLst
            wl_eATZON = New moe_Estructuras.eATZON
            wg_sTblCon1 = "ATZON"
            wl_frmLst.ShowDialog()
            If wl_frmLst.wp_eATZON.sCodZon <> "" Then
                wl_eATZON = wl_frmLst.wp_eATZON
            End If
        Catch ex As Exception
            MsgBox("Error :  BD_frmATZON")
            MsgBox(ex.Message)
        End Try
    End Sub
#End Region
...
#Region "CommandButtons_Click ABRECO Controladores de Eventos (Remitente_evento)"
    ...
    Private Sub cmdBuscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdBuscar.Click
        Try
            ...
            wl_bYesNo = False
            mof_LmpMsgError()
            vl_sCodZon = txtsCodZon.Text
            If txtsCodZon.Text <> "" Then
                BD_DatPrmTipTbl(wl_sTipTbl)
                wl_sCmpOrdE11 = ""
                wl_sTipOrdE11 = ""
                wr_bEOFE1 = BD_BuscarATZON()
                If wr_bEOFE1 Then
                    wp_sNomTbl = wl_sNomTblE1
                    wp_sDatCmp = txtsCodZon.Text
    ...
    Private Sub cmdCrear_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdCrear.Click
        Try
            ...
            If txtsCodZon.Text <> "" Then
                vl_sCodZon = txtsCodZon.Text
                BD_DatPrmTipTbl(wl_sTipTbl)
                wr_bEOFE1 = mof_fConATPOP(vl_sCodZon,
                                          wl_eATZON)
                If wr_bEOFE1 Then
                    wl_bYesNo = BD_CrearActualizar(wp_sCmdSel)
                    If wl_bYesNo Then
            ...
            If txtsCodZon.Text <> "" Then
                ...
                If wp_nResMsg = 6 Then
                    ...
                    If Not wr_bEOFE1 Then
                        BD_eATZON_DT(wl_DTE1)
                        wl_bVda = False
                        wp_sNomTbl = ""
                        wl_bYesNo = mov_VdasCodZon(vl_sCodZon,
                                                   wp_sNomTbl,
                                                   wl_bVda)
                        If wl_bVda = True Then
                            wl_bYesNo = clnATZON.cnEliATZON(wl_eATZON,
                                                            wr_bEOF)
                            If wl_bYesNo Then
...
#Region "060 CommandButtons_Click Lista Controladores de Eventos (Remitente_evento)"
    Private Sub cmdLstATZON_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdLstATZON.Click
        Try
            Dim wl_frmLst As New frmGQLst
            BD_frmATZON()
            If wl_eATZON.sCodZon <> "" Then
                BD_Var_eATZON()
                Plo_Frm_Var()
                txtsNomZon.Focus()
                mof_ActivarOK(Me)
            ...
            End If
        Catch ex As Exception
            MsgBox("Error en: cmdLstATPOP_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
...
#Region "080 Plo Procedimientos del Formulario. Personalizados"
    ...
    Private Sub Plo_IniciarVar()
        Try
            wl_bYesNo = False
            wl_sCodErrAdi = "10-311"
            wl_sCodErrCon = "10-111"
            wl_sCodErrOk = "20-001"
    ...
    Private Sub Plo_Frm_Var()
        Try
            txtsCodZon.Text = vl_sCodZon
            txtsNomZon.Text = vl_sNomZon
...
#Region "081 Plo Procedimientos del Formulario. Comunes"
    ...
    Private Sub Plo_ActivarCmpAct()
        Try
            txtsCodZon.Enabled = True
            txtsNomZon.Enabled = True
...
#Region "082 Elo Eventos locales del Formulario"
    Private Sub txtsCodZon_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        Try
...
#Region "110 grl Changed"
    ...
    Private Sub grl_Txt_Changed(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If DirectCast(sender, TextBox).Name = "txtsCodZon" Then
            wl_sClaPpl = txtsCodZon.Text
            mof_LmpMsgError()
        Else
            grl_TextChange()
        End If
    ...
    Private Sub grl_Txt_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        ...
        If cmdCancelar.Focused = False Then
            If wl_cvTxt IsNot Nothing Then
                If wl_cvTxt.Name = "txtsCodZon" Then
                    mov_VdaTxt(wl_bVda,
                               txtsCodZon)
                End If
            End If
