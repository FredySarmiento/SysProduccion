...
#Region "012 BD - Variables campos BD Locales"
    Dim vl_nId As Long
    Dim vl_sCodEstODP As String
    Dim vl_sNomEstODP As String
...
#Region "014 Objetos locales"
    Dim clnABEOP As New ClaseNegocios.clnABEOP
...
#Region "020 frm Eventos - Principal"
    Private Sub frmABEOP_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Disposed
        Try
            grl_RemoveHandler()
        Catch ex As Exception
            MsgBox("Error : frmABEOP_Disposed")
            MsgBox(ex.Message)
        End Try
    End Sub

    Private Sub frmABEOP_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            Me.Text += "  -  " + vg_sNomCia
            grl_RemoveHandler()
            grl_AddHandler()
...
#Region "040 BD Procedimientos sobre la BD. Personalizados"
    Private Function BD_CrearActualizar(ByVal wp_sCmdSel As String) As Boolean
        ...
        Try
            Plo_Var_Frm()
            BD_eABEOP_Var()
            wl_bYesNo = clnABEOP.cnCrearActABEOP(wl_eABEOP,
                                                 wr_bEOFE1,
                                                 wp_sCmdSel)
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub BD_DatPrmTipTbl(ByRef wl_sTipTbl As String)
        Try
            wl_sNomTblE1 = "ABEOP"
            wl_sNomCmpE11 = "sCodEstODP "
            wl_sTipCmpE11 = "C"
            wl_sTitCmpE11 = "Código"
            wl_sDatCmpE11 = txtsCodEstODP.Text
    ...
    Private Sub BD_DatPrmTSB() 'Constante
        ...
        End Try
    End Sub
    Private Sub BD_eABEOP_Var()
        Try
            With wl_eABEOP
                .nId = vl_nId
                .sCodEstODP = vl_sCodEstODP
                .sNomEstODP = vl_sNomEstODP
                .bRegAci = vl_bRegAci
                .nNroOrd = vl_nNroOrd
        ...
        Catch ex As Exception
            MsgBox(ex.Message)
            MsgBox("Error en el SUB: BD_eABEOP_Var")
        End Try
    End Sub
    Private Sub BD_Var_eABEOP()
        Try
            With wl_eABEOP
                vl_sCodEstODP = .sCodEstODP
                vl_sNomEstODP = .sNomEstODP
                vl_bRegAci = .bRegAci
                vl_nNroOrd = .nNroOrd
            End With
        ...
        Catch ex As Exception
            MsgBox(ex.Message)
            MsgBox("Error en el SUB: BD_Var_eABEOP")
        End Try
    End Sub
    Private Sub BD_eABEOP_DT(ByVal wp_DTE1 As DataTable)
        Try
            ...
            moo_eABEOP_DT(wl_eABEOP,
                          wp_DTE1,
                          wl_nRow)
            wl_sDatCmpE12 = txtsCodEstODP.Text

        Catch ex As Exception
            MsgBox("Error :  BD_eABEOP_DT")
            MsgBox(ex.Message)
        End Try
    End Sub
#End Region
#Region "041 BD Procedimientos sobre la BD. Comunes"
    Private Sub BD_tsbPASU()
        Try
            Plo_ActivarCmpAct()
            txtsCodEstODP.Enabled = False
            vl_sCodEstODP = wl_DTE1.Rows(0).Item("sCodEstODP ")
            wl_sDatCmpE11 = vl_sCodEstODP
            BD_BuscarABEOP()
            BD_Var_eABEOP()
            Plo_Frm_Var()
            mof_ActivarOK(Me)
...
#Region "042 BD Busquedas"
    Private Function BD_BuscarABEOP() As Boolean
        Try
            wr_bEOFE2 = True
            wl_eABEOP = New moe_Estructuras.eABEOP
            wr_bEOFE2 = mof_fConABEOP(vl_sCodEstODP,
                                      wl_eABEOP)
        Catch ex As Exception
            MsgBox(ex.Message)
            MsgBox("Error en el SUB: BD_BuscarABEOP")
        End Try
        Return wr_bEOFE2
    End Function
...
#Region "CommandButtons_Click ABRECO Controladores de Eventos (Remitente_evento)"
    ...
    Private Sub cmdBuscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdBuscar.Click
        Try
            ...
            wl_bYesNo = False
            mof_LmpMsgError()
            vl_sCodEstODP = txtsCodEstODP.Text
            If txtsCodEstODP.Text <> "" Then
                BD_DatPrmTipTbl(wl_sTipTbl)
                wl_sCmpOrdE11 = ""
                wl_sTipOrdE11 = ""
                BD_BuscarABEOP()
    ...
    Private Sub cmdCrear_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdCrear.Click
        Try
            ...
            If txtsCodEstODP.Text <> "" Then
                vl_sCodEstODP = txtsCodEstODP.Text
                BD_DatPrmTipTbl(wl_sTipTbl)
                wr_bEOFE1 = mof_fConABEOP(vl_sCodEstODP,
                                          wl_eABEOP)
                If wr_bEOFE1 Then
                    wl_bYesNo = BD_CrearActualizar(wp_sCmdSel)
                    If wl_bYesNo Then
            ...
            If txtsCodEstODP.Text <> "" Then
                ...
                If wp_nResMsg = 6 Then
                    ...
                    If Not wr_bEOFE1 Then
                        BD_eABEOP_DT(wl_DTE1)
                        wl_bVda = False
                        wp_sNomTbl = ""
                        wl_bYesNo = mov_VdasCodOperario(vl_sCodEstODP,
                                                        wp_sNomTbl,
                                                        wl_bVda)
                        If wl_bVda = True Then
                            wl_bYesNo = clnABEOP.cnEliABEOP(wl_eABEOP,
                                                            wr_bEOF)
                            If wl_bYesNo Then
...
#Region "060 CommandButtons_Click Lista Controladores de Eventos (Remitente_evento)"
    Private Sub cmdLstABEOP_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdLstABEOP.Click
        Try
            Dim wl_frmLst As New frmGQLst
            BD_frmABEOP()
            If wl_eABEOP.sCodEstODP <> "" Then
                BD_Var_eABEOP()
                Plo_Frm_Var()
                txtsNomEstODP.Focus()
                mof_ActivarOK(Me)
            ...
            End If
        Catch ex As Exception
            MsgBox("Error en: cmdLstABEOP_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
...
#Region "080 Plo Procedimientos del Formulario. Personalizados"
    ...
    Private Sub Plo_Frm_Var()
        Try
            txtsCodEstODP.Text = vl_sCodEstODP
            txtsNomEstODP.Text = vl_sNomEstODP
    ...
    Private Sub Plo_Var_Frm()
        Try
            vl_sCodEstODP = txtsCodEstODP.Text
            vl_sNomEstODP = txtsNomEstODP.Text
...
#Region "081 Plo Procedimientos del Formulario. Comunes"
    ...
    Private Sub Plo_ActivarCmpAct()
        Try
            txtsCodEstODP.Enabled = True
            txtsNomEstODP.Enabled = True
...
#Region "082 Elo Eventos locales del Formulario"
    Private Sub txtsCodEstODP_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtsCodEstODP.KeyUp
        Try
