...
#Region "020 frm Eventos - Principal"
    ...
    Private Sub frmATOPE_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Disposed
        Try
            grl_RemoveHandler()
    ...
    Private Sub frmATOPE_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            Me.Text += "  -  " + vg_sNomCia
            grl_RemoveHandler()
            grl_AddHandler()
...
#Region "040 BD Procedimientos sobre la BD. Personalizados"
    Private Function BD_CrearActualizar(ByVal wp_sCmdSel As String) As Boolean
        ...
        Try
            Plo_Var_Frm()
            BD_eATOPE_Var()
            wl_bYesNo = clnATOPE.cnCrearActATOPE(wl_eATOPE,
                                                 wr_bEOFE1,
                                                 wp_sCmdSel)
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub BD_DatPrmTipTbl(ByRef wl_sTipTbl As String)
        Try
            wl_sNomTblE1 = "ATOPE"
            wl_sNomCmpE11 = "sCodOpe"
            wl_sTipCmpE11 = "C"
            wl_sTitCmpE11 = "Código Ope "
            wl_sDatCmpE11 = txtsCodOpe.Text
    ...
    Private Sub BD_Var_eATOPE()
        ...
        Catch ex As Exception
            MsgBox(ex.Message)
            MsgBox("Error en el SUB: BD_Var_eATOPE")
        End Try
    End Sub
    Private Sub BD_eATOPE_DT(ByVal wp_DTE1 As DataTable)
        Try
            Dim wl_nRow As Integer
            wl_nRow = wp_DTE1.Rows.Count - 1
            wl_eATOPE = New moe_Estructuras.eATOPE
            moo_eATOPE_DT(wl_eATOPE,
                          wp_DTE1,
                          wl_nRow)

            vl_sCodCtr = wl_eATCTR.sCodCtr
            BD_BuscarATCTR()
            cboATCTR.SelectedValue = wl_eATOPE.sCodOpe
            wl_sDatCmpE12 = txtsCodOpe.Text

        Catch ex As Exception
            MsgBox("Error :  BD_eATOPE_DT")
            MsgBox(ex.Message)
        End Try
    End Sub
#End Region
#Region "041 BD Procedimientos sobre la BD. Comunes"
    Private Sub BD_tsbPASU()
        Try
            Plo_ActivarCmpAct()
            txtsCodOpe.Enabled = False
            vl_sCodOpe = wl_DTE1.Rows(0).Item("sCodOpe")
            wl_sDatCmpE11 = vl_sCodOpe
            BD_BuscarATOPE()
            BD_Var_eATOPE()
            Plo_Frm_Var()
            mof_ActivarOK(Me)
...
#Region "042 BD Busquedas"
    Private Function BD_BuscarATCTR() As Boolean
        Try
            ...
            wr_bEOFE2 = mof_fConATCTR(vl_sCodOpe,
                                      wl_eATCTR)
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub cmdCrear_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdCrear.Click
        Try
            ...
            If txtsCodOpe.Text <> "" Then
                vl_sCodOpe = txtsCodOpe.Text
                BD_DatPrmTipTbl(wl_sTipTbl)
                wr_bEOFE1 = mof_fConATOPE(vl_sCodOpe,
                                          wl_eATOPE)
                If wr_bEOFE1 Then
                    wl_bYesNo = BD_CrearActualizar(wp_sCmdSel)
                    If wl_bYesNo Then
            ...
            If txtsCodOpe.Text <> "" Then
                ...
                If wp_nResMsg = 6 Then
                    ...
                    If Not wr_bEOFE1 Then
                        BD_eATOPE_DT(wl_DTE1)
                        wl_bVda = False
                        wp_sNomTbl = ""
                        wl_bYesNo = mov_VdasCodOpe(vl_sCodOpe,
                                                   wp_sNomTbl,
                                                   wl_bVda)
                        If wl_bVda = True Then
                            wl_bYesNo = clnATOPE.cnEliATOPE(wl_eATOPE,
                                                            wr_bEOF)
                            If wl_bYesNo Then
...
#Region "060 CommandButtons_Click Lista Controladores de Eventos (Remitente_evento)"
    Private Sub cmdLstATOPE_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdLstATOPE.Click
        Try
            Dim wl_frmLst As New frmGQLst
            BD_frmATOPE()
...
#Region "080 Plo Procedimientos del Formulario. Personalizados"
    ...
    Private Sub Plo_Frm_Var()
        Try
            txtsCodOpe.Text = vl_sCodOpe
            txtsNomOpe.Text = vl_sNomOpe
            cboATCTR.SelectedValue = vl_sCodCtr
            txtnTieOpe.Text = vl_nTieOpe
    ...
    Private Sub Plo_Var_Frm()
        Try
            ...
            vl_nTieOpe = CDbl(txtnTieOpe.Text)
            vl_nUniTieOpe = CLng(txtnUniTieOpe.Text)
...
#Region "081 Plo Procedimientos del Formulario. Comunes"
    ...
    Private Sub Plo_ActivarCmpAct()
        Try
            txtsCodOpe.Enabled = True
            txtsNomOpe.Enabled = True
            cboATCTR.Enabled = True
            txtnTieOpe.Enabled = True
    ...
    Private Sub Plo_InactivarCmpAct()
        Try
            txtsCodOpe.Enabled = False
            txtsNomOpe.Enabled = False
            cboATCTR.Enabled = False
            txtnTieOpe.Enabled = False
...
#Region "110 grl Changed"
    ...
    Private Sub grl_Txt_Changed(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If DirectCast(sender, TextBox).Name = "txtsCodOpe" Then
            wl_sClaPpl = txtsCodOpe.Text
            mof_LmpMsgError()
        Else
            grl_TextChange()
        End If
    ...
    Private Sub grl_Txt_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        ...
        If cmdCancelar.Focused = False Then
            If wl_cvTxt IsNot Nothing Then
                If wl_cvTxt.Name = "txtsCodOpe" Then
                    mov_VdaTxt(wl_bVda,
                               txtsCodOpe)
                End If
            End If
