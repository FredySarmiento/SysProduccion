...
#Region "020 frm Eventos - Principal"
    ...
    Private Sub frmATPOP_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Disposed
        Try
            grl_RemoveHandler()
        Catch ex As Exception
            MsgBox("Error : frmATPOP_Disposed")
            MsgBox(ex.Message)
        End Try
    End Sub

    Private Sub frmATPOP_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            Me.Text += "  -  " + vg_sNomCia
            grl_RemoveHandler()
            grl_AddHandler()
...
#Region "040 BD Procedimientos sobre la BD. Personalizados"
    Private Function BD_CrearActualizar(ByVal wp_sCmdSel As String) As Boolean
        ...
        Try
            Plo_Var_Frm()
            BD_eATPOP_Var()
            wl_bYesNo = clnATPOP.cnCrearActATPOP(wl_eATPOP,
                                                 wr_bEOFE1,
                                                 wp_sCmdSel)
        Catch ex As Exception
            MsgBox(ex.Message)
    ...
    Private Sub BD_DatPrmTipTbl(ByRef wl_sTipTbl As String)
        Try
            wl_sNomTblE1 = "ATPOP"
            wl_sNomCmpE11 = "sCodTer"
            wl_sTipCmpE11 = "C"
            wl_sTitCmpE11 = "Código Ter "
            wl_sDatCmpE11 = txtsCodTer.Text
    ...
    Private Sub BD_Var_eATPOP()
        ...
        Catch ex As Exception
            MsgBox(ex.Message)
            MsgBox("Error en el SUB: BD_Var_eATPOP")
        End Try
    End Sub
    Private Sub BD_eATPOP_DT(ByVal wp_DTE1 As DataTable)
        Try
            ...
            moo_eATPOP_DT(wl_eATPOP,
                          wp_DTE1,
                          wl_nRow)
            wl_sDatCmpE12 = txtsCodTer.Text

        Catch ex As Exception
            MsgBox("Error :  BD_eATPOP_DT")
            MsgBox(ex.Message)
        End Try
    End Sub
#End Region
#Region "041 BD Procedimientos sobre la BD. Comunes"
    Private Sub BD_tsbPASU()
        Try
            ...
            vl_sCodTer = wl_DTE1.Rows(0).Item("sCodTer")
            wl_sDatCmpE11 = vl_sCodTer
            BD_BuscarATPOP()
            BD_Var_eATPOP()
            Plo_Frm_Var()
            mof_ActivarOK(Me)
...
#Region "045 BD Llamar frm"
    Private Sub BD_frmATPOP()
        Try
            Dim wl_frmLst As New frmGQLst
            wl_eATPOP = New moe_Estructuras.eATPOP
            wg_sTblCon1 = "ATPOP"
            wl_frmLst.ShowDialog()
            If wl_frmLst.wp_eATPOP.sCodTer <> "" Then
    ...
    Private Sub cmdCrear_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdCrear.Click
        Try
            ...
            If txtsCodTer.Text <> "" Then
                vl_sCodTer = txtsCodTer.Text
                BD_DatPrmTipTbl(wl_sTipTbl)
                wr_bEOFE1 = mof_fConATPOP(vl_sCodTer,
                                          wl_eATPOP)
                If wr_bEOFE1 Then
                    wl_bYesNo = BD_CrearActualizar(wp_sCmdSel)
                    If wl_bYesNo Then
            ...
            If txtsCodTer.Text <> "" Then
                ...
                If wp_nResMsg = 6 Then
                    ...
                    If Not wr_bEOFE1 Then
                        BD_eATPOP_DT(wl_DTE1)
                        wl_bVda = False
                        wp_sNomTbl = ""
                        wl_bYesNo = mov_VdasCodOperario(vl_sCodTer,
                                                        wp_sNomTbl,
                                                        wl_bVda)
                        If wl_bVda = True Then
                            wl_bYesNo = clnATPOP.cnEliATPOP(wl_eATPOP,
                                                            wr_bEOF)
                            If wl_bYesNo Then
...
#Region "080 Plo Procedimientos del Formulario. Personalizados"
    ...
    Private Sub Plo_IniciarVar()
        Try
            wl_bYesNo = False
            wl_sCodErrAdi = "10-322"
            wl_sCodErrCon = "10-122"
            wl_sCodErrOk = "20-001"
...
#Region "082 Elo Eventos locales del Formulario"
    Private Sub txtsCodOpe_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtsCodTer.KeyUp
        Try
