...
#Region "020 frm Eventos - Principal"
    ...
    Private Property wl_sCodItmIni As String

    Private Sub frmIEENI_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Disposed
        Try
            grl_RemoveHandler()
            Me.Dispose()
        ...
        End Try
    End Sub
    Private Sub frmIEENI_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            Me.Text += "  -  " + vg_sNomCia
            grl_RemoveHandler()
            grl_AddHandler()
...
#Region "040 BD Procedimientos sobre la BD. Personalizados"
    ...
    Private Sub BD_DatPrmTipTbl(ByRef wg_sTipTbl As String)
        ...
        Try
            ...
            If wg_sTipTbl = "E2" Then
                ...
                'wg_sDatCmpE12 = CType(txtnNroDoc.Text, Double)
    ...
    Private Sub BD_eGEMOV_DT(ByVal wp_DTE1 As DataTable)
        ...
        End Try
    End Sub
    Private Sub BD_eGEMOV_nNroDocOri(ByVal wl_eGEMOV As Object)
        Try
            With wl_eGEMOV
                .sCodDoc = vl_sCodDoc
                .nNroDoc = wl_nNroDocOri
                .sCodser = vl_sCodSer
                .sEstDoc = vl_sEstDoc
                .dFecCtb = vl_dFecCtb
                .dFecDoc = vl_dFecDoc
...
#Region "050 CommandButtons_Click ABRECO Controladores de Eventos (Remitente_evento)"
    Private Sub cmdActualizar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdActualizar.Click
        Try
            '2018-09-18
            'Solamente se actualiza en encabezado
...
#Region "060 CommandButtons_Click Lista Controladores de Eventos (Remitente_evento)"
    Private Sub cmdLstIEENI_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdLstIEENI.Click
        Try
            Dim wl_frmLst As New frmGQLst
            wg_sTblCon1 = "IEENI"
            wl_eGEMOV = New moe_Estructuras.eGEMOV
    ...
    Private Sub cmdCpd_Click(sender As System.Object, e As System.EventArgs) Handles cmdCpd.Click
        Try
            Dim wl_frmIEENI_LeeENT As New frmIEENI_LeeENT


            wl_frmIEENI_LeeENT.wp_sCodDocOri = vl_sCodDoc
            wl_frmIEENI_LeeENT.wp_nNroDocOri = 0
            wl_sCodDocIni = vl_sCodDoc
            wl_nNroDocIni = CInt(txtnNroDoc.Text)
            vl_nNroDoc = CInt(txtnNroDoc.Text)
            wl_nNroDocOri = 0

            wl_frmIEENI_LeeENT.ShowDialog()
            wl_nNroDocOri = CInt(wl_frmIEENI_LeeENT.wp_nNroDocOri)

            If wl_nNroDocOri > 0 Then
                wl_eGEMOV = New moe_Estructuras.eGEMOV
                wl_eGEMOV.sCodDoc = wl_sCodDocIni
                wl_eGEMOV.sCodSer = vl_sCodSer
                wl_eGEMOV.nNroDoc = wl_nNroDocOri
                BD_DatPrmTipTbl(wg_sTipTbl)
                wg_DTE1 = clnIEINV.cndtConGEMOV(wl_sNomTblE1,
                                                wl_eGEMOV,
                                                wr_bEOFE1)
                ...
                Else
                    ...
                    BD_eGEMOV_DT(wg_DTE1)
                    BD_eGEMOV_nNroDocOri(wl_eGEMOV)
                    BD_Var_eGEMOV()
                    'Plo_Frm_Var()
...
#Region "070 DGV Procedimientos del Usuario. Personalizados"
    ...
    Private Sub dgv_LstADgv()
        ...
        End Try
    End Sub
    Private Sub dgv_Vda_SdoAlm(ByVal wp_nRow)
        Try
            If dgv1.Rows(wp_nRow).Cells("bIndCan").Value = True Then
                If dgv1.Rows(wp_nRow).Cells("nCanItm").Value > dgv1.Rows(wp_nRow).Cells("nSdoAlm").Value Then
                    dgv1.Rows(wp_nRow).DefaultCellStyle.ForeColor = Color.Red
                Else
                    dgv1.Rows(wp_nRow).DefaultCellStyle.ForeColor = Color.Black
                End If
            End If
            If dgv1.Rows(wp_nRow).DefaultCellStyle.ForeColor = Color.Black Then
                If dgv1.Rows(wp_nRow).Cells("bIndCos").Value = True Then
                    If dgv1.Rows(wp_nRow).Cells("nPreUni").Value <= 0 Then
                        dgv1.Rows(wp_nRow).DefaultCellStyle.ForeColor = Color.Red
                    Else
                        dgv1.Rows(wp_nRow).DefaultCellStyle.ForeColor = Color.Black
                    End If
...
#Region "071 DGV Procedimientos del Usuario. Comunes"
    ...
    Private Sub dgv_EliminarLinea()
        Dim wl_nRow As Integer
        Try
            wl_nRow = (wl_CtrlDgv.Rows.Count).ToString
            wl_nCol = wl_CtrlDgv.CurrentRow.Index
            If wl_nRow >= 0 Then
...
#Region "090 Rpt - Reportes"
    Private Sub Rpt_PrmRpt()
         Try
            ...
            mof_ActivarOK(Me)
            Dim wl_frmRPT As New rptIRENI01
