Public Class frmPRODP05
#Region "000 Inicia. Definición de variables"
    'Ultima actualización 000 - 019 : 2015-03-09

#Region "001 Inicia. wp - vp Definición del variables públicas"
#End Region
#Region "010 Inicia. vl - wl Definición del variables Locales"
    ' vl - Variables que corresponde a campos de la BD locales
    ' wl - Variables de trabajo locales
    ' wp - Variables de trabajo locales parámetro
    ' vp - Variables que corresponde a campos de la BD locales - parámeto
#End Region
#Region "011 frm - Variables Locales"
    ' vl numéricas

    ' wl Booleanas
    Dim wl_bYesNo As Boolean
    Dim wl_bVda As Boolean
    Dim wl_bEOF As Boolean


    ' wl numéricas
    Dim wl_sNomTblCbo As String
    Dim wl_sNomFrm As String

    Dim wl_nCmpCla As String

    Dim wl_sCmpTSB As String
    Dim wl_sDatTSB As String
    Dim wl_sTipCmpTSB As String

    Dim wl_sCmpTSB1 As String
    Dim wl_sDatTSB1 As String
    Dim wl_sTipCmpTSB1 As String

    Dim wl_sCmpTSB2 As String
    Dim wl_sDatTSB2 As String
    Dim wl_sTipCmpTSB2 As String

    Dim wp_sTipCmp As String
    Dim wl_sTipTbl As String

    Dim wl_sClaPpl As String
    Dim wl_sClaPPl1 As String
    Dim wl_sClaPPl2 As String
    Dim wl_sDatCmpE11 As String
    Dim wl_sNomTblE1 As String
    Dim wl_sNomCmpE11 As String
    Dim wl_sTipCmpE11 As String
    Dim wl_sTitCmpE11 As String
    Dim wl_sDatCmpE12 As String
    Dim wl_sNomTblE2 As String
    Dim wl_sNomCmpE12 As String
    Dim wl_sTipCmpE12 As String
    Dim wl_sTitCmpE12 As String
    Dim wl_sCmpOrdE11 As String
    Dim wl_sTipOrdE11 As String

    Dim wl_sNroALetras As String

#End Region
#Region "012 BD - Variables campos BD Locales"
    Dim vl_nNroDocFin As Long
    Dim vl_nNroDocIni As Long

    Dim vl_sCodDocFin As String
    Dim vl_sCodDocIni As String
    Dim vl_sEstOdpFin As String
    Dim vl_sEstOdpIni As String
    Dim vl_sCodSerFin As String
    Dim vl_sCodSerIni As String
    Dim vl_sCodItmFin As String
    Dim vl_sCodItmIni As String
    Dim wl_sCodDoc As String
    Dim wl_sCodSer As String
    Dim wl_sEstOdp As String
    Dim wl_nNroDoc As Long
    Dim vl_sCodDoc As String
    Dim wl_sCodItm As String
    Dim vl_sCodStr As String
    Dim vl_sTipMov As String


#End Region
#Region "013 DGV - Variables Locales"
    'booleanas
    'cadena
    ' Dim wl_sOpeLgc As String  'Y, O, N

#End Region
#Region "014 Objetos locales"
    Dim clnIEDMI As New ClaseNegocios.clnIEDMI
    Dim clnIEINV As New ClaseNegocios.clnIEINV
    Dim clnPEODP As New ClaseNegocios.clnPEODP
    Dim clnPEROP As New ClaseNegocios.clnPEROP
    Dim cnf As New ClaseNegocios.clnFunciones
    Dim clnTblAB As New ClaseNegocios.clnTblAB


    Dim wl_DTE1 As DataTable
    Dim wl_DTE2 As DataTable
    Dim wl_DTL1 As DataTable
    Dim wl_DTL2 As DataTable
#End Region
#Region "015 Estructuras"
    Dim wl_aeABEOP() As moe_Estructuras.eABEOP
    Dim wl_aeABODP() As moe_Estructuras.eABODP
    Dim wl_aeAPLND1() As moe_Estructuras.eAPLND1
    Dim wl_aePEODP_PLODP4_Ini() As moe_Estructuras.ePEODP_PLODP4
    Dim wl_aePEODP_PLODP4_Fin() As moe_Estructuras.ePEODP_PLODP4
    Dim wl_aePEODP_PLODP4() As moe_Estructuras.ePEODP_PLODP4
    Dim wl_aePEODP_PLODP4_QGrl() As moe_Estructuras.ePEODP_PLODP4

    Dim wl_eABEOP As New moe_Estructuras.eABEOP
    Dim wl_eABODP As New moe_Estructuras.eABODP
    Dim wl_eAPLND1 As New moe_Estructuras.eAPLND1
    Dim wl_ePEODP_PLODP4_Ini As New moe_Estructuras.ePEODP_PLODP4
    Dim wl_ePEODP_PLODP4_Fin As New moe_Estructuras.ePEODP_PLODP4
    Dim wl_ePEODP_PLODP4 As New moe_Estructuras.ePEODP_PLODP4
    Dim wl_ePEODP_PLODP4_QGrl As New moe_Estructuras.ePEODP_PLODP4


    Dim wp_aePEODP_PLODP4(0) As moe_Estructuras.ePEODP_PLODP4
    Dim wp_ePEODP_PLODP4 As New moe_Estructuras.ePEODP_PLODP4
    Dim wp_ePEODP_PLODP4_QGrl As New moe_Estructuras.ePEODP_PLODP4

#End Region
#Region "018 Errores"
    Dim wl_sCoderr As String
    Dim wl_sDatCmp As String
    Dim wl_sNomtbl As String
    Dim wl_sCodErrAdi As String
    Dim wl_sCodErrCon As String
    Dim wl_sCodErrOk As String
    Dim wl_sDesMsgCmd As String
    Dim wl_sTitMsgCmd As String
#End Region
#Region "019 Fin definición del variables Locales"
#End Region
#End Region
#Region "020 frm Eventos - Principal"
    Private Sub frmPRODP05_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Try
            Me.Text += "  -  " + vg_sNomCia
            mof_fCnfCtrl(Me)
            mof_IniciarVar()
            mof_fLmpCtrl(Me)
            Plo_IniciarFrm()
            Plo_IniciarEst()
            Plo_IniciarVar()

        Catch ex As Exception
            MsgBox("Error :  frmRPODP01_Load")
            MsgBox(ex.Message)
        End Try
    End Sub
#End Region

#Region "030 Cbo Llenar Combobox. Funciones"
    Private Sub Cbo_CargarLst()
        Try
        Catch ex As Exception
            MsgBox("Error :  Cbo_CargarLst")
            MsgBox(ex.Message)
        End Try
    End Sub
#End Region
#Region "040 BD"

#Region "040 BD Procedimientos sobre la BD. Personalizados"
   
#End Region

#Region "041 BD Procedimientos sobre la BD. Comunes"

#End Region
#Region "042 BD Busquedas"
    

    Private Function BD_ConPEODP_PLODP4() As Boolean
        Try
            wr_bEOFE2 = True
            wl_DTE1 = New DataTable
            wl_DTE1 = clnPEODP.cndtConPEODP_PLODP4(wl_ePEODP_PLODP4,
                                                   wr_bEOFE2)
        Catch ex As Exception
            MsgBox(ex.Message)
            MsgBox("Error en el SUB: BD_ConPEODP_PLODP4")
        End Try
        Return wr_bEOFE2

    End Function
   

#End Region
#Region "043 BD Cargar"

#End Region
#Region "045 BD Llamar frm"
    Private Sub BD_frmABEOP()
        Try
            Dim wl_frmLst As New frmGQLst
            wl_eABEOP = New moe_Estructuras.eABEOP
            wl_frmLst.wp_eABEOP = New moe_Estructuras.eABEOP
            wg_sTblCon1 = "ABEOP"
            wl_frmLst.ShowDialog()
            If wl_frmLst.wp_eABEOP.sCodEstODP <> "" Then
                wl_eABEOP = wl_frmLst.wp_eABEOP
            End If
        Catch ex As Exception
            MsgBox("Error :  BD_frmABEOP")
            MsgBox(ex.Message)
        End Try

    End Sub
    Private Sub BD_frmABODP()
        Try
            Dim wl_frmLst As New frmGQLst
            wl_eABODP = New moe_Estructuras.eABODP
            wl_frmLst.wp_eABODP = New moe_Estructuras.eABODP
            wg_sTblCon1 = "ABODP"
            wl_frmLst.ShowDialog()
            If wl_frmLst.wp_eABODP.sCodDoc <> "" Then
                wl_eABODP = wl_frmLst.wp_eABODP
            End If
        Catch ex As Exception
            MsgBox("Error :  BD_frmABODP")
            MsgBox(ex.Message)
        End Try

    End Sub
    Private Sub BD_frmAPLND1()
        Try
            Dim wl_frmLst As New frmGQLst
            wl_eAPLND1 = New moe_Estructuras.eAPLND1
            wl_frmLst.wp_eAPLND1 = New moe_Estructuras.eAPLND1
            wl_frmLst.vp_sCodDoc = wl_sCodDoc
            wl_frmLst.vp_sCodSer = ""
            wg_sTblCon1 = "APLND1"
            wl_frmLst.ShowDialog()
            If wl_frmLst.wp_eAPLND1.sCodDoc <> "" Then
                wl_eAPLND1 = wl_frmLst.wp_eAPLND1
            End If
        Catch ex As Exception
            MsgBox("Error :  BD_frmAPLND1")
            MsgBox(ex.Message)
        End Try

    End Sub
    Private Sub BD_frmPEODP_PLODP4()
        Try
            Dim wl_frmLst As New frmGQLst
            wl_ePEODP_PLODP4 = New moe_Estructuras.ePEODP_PLODP4
            wl_frmLst.wp_ePEODP = New moe_Estructuras.ePEODP
            wl_frmLst.vp_sCodDoc = wl_sCodDoc
            wl_frmLst.vp_sEstOdp = ""
            wg_sTblCon1 = "PEODP_PLODP4"
            wl_frmLst.ShowDialog()
            If wl_frmLst.wp_ePEODP_PLODP4.sCodDoc <> "" Then
                wl_ePEODP_PLODP4 = wl_frmLst.wp_ePEODP_PLODP4
            End If
        Catch ex As Exception
            MsgBox("Error :  BD_frmPEODP_PLODP4")
            MsgBox(ex.Message)
        End Try

    End Sub
    Private Sub BD_frmPEODP_PLODP4_QGrl()
        Try
            Dim wl_frmLst As New frmGQLst
            wl_frmLst.wp_ePEODP_PLODP4_QGrl = New moe_Estructuras.ePEODP_PLODP4
            wl_frmLst.wp_ePEODP_PLODP4 = wl_ePEODP_PLODP4
            wl_frmLst.vp_sCodStr = vl_sCodStr
            wg_sTblCon1 = "PEODP_PLODP4_QGrl"

            wl_frmLst.ShowDialog()
            If wl_frmLst.wp_ePEODP_PLODP4_QGrl.sCodDoc <> "" Then
                wp_ePEODP_PLODP4_QGrl.sCodDoc = wl_frmLst.wp_ePEODP_PLODP4_QGrl.sCodDoc
            End If
            If wl_frmLst.wp_ePEODP_PLODP4_QGrl.sCodSer <> "" Then
                wl_ePEODP_PLODP4_QGrl.sCodSer = wl_frmLst.wp_ePEODP_PLODP4_QGrl.sCodSer
            End If
            If wl_frmLst.wp_ePEODP_PLODP4_QGrl.nNroDoc <> 0 Then
                wl_ePEODP_PLODP4_QGrl.nNroDoc = wl_frmLst.wp_ePEODP_PLODP4_QGrl.nNroDoc
            End If

            If wl_frmLst.wp_ePEODP_PLODP4_QGrl.sEstOdp <> "" Then
                wp_ePEODP_PLODP4_QGrl.sEstOdp = wl_frmLst.wp_ePEODP_PLODP4_QGrl.sEstOdp
            End If
            If wl_frmLst.wp_ePEODP_PLODP4_QGrl.sCodItm <> "" Then
                wp_ePEODP_PLODP4_QGrl.sCodItm = wl_frmLst.wp_ePEODP_PLODP4_QGrl.sCodItm
            End If

            If wl_frmLst.wp_ePEODP_PLODP4_QGrl.dFecEmi <> Nothing Then
                wp_ePEODP_PLODP4_QGrl.dFecEmi = wl_frmLst.wp_ePEODP_PLODP4_QGrl.dFecEmi
            End If
            If wl_frmLst.wp_ePEODP_PLODP4_QGrl.dFecIni <> Nothing Then
                wp_ePEODP_PLODP4_QGrl.dFecIni = wl_frmLst.wp_ePEODP_PLODP4_QGrl.dFecIni
            End If
            If wl_frmLst.wp_ePEODP_PLODP4_QGrl.dFecFin <> Nothing Then
                wp_ePEODP_PLODP4_QGrl.dFecFin = wl_frmLst.wp_ePEODP_PLODP4_QGrl.dFecFin
            End If



        Catch ex As Exception
            MsgBox("Error :  BD_frmPEODP_PLODP4_QGrl")
            MsgBox(ex.Message)
        End Try

    End Sub
    Private Sub BD_frmPEODP_PLODP4_QIniFin()
        Try
            Dim wl_frmLst As New frmGQLst
            wl_frmLst.wp_ePEODP_PLODP4_Ini = New moe_Estructuras.ePEODP_PLODP4
            wl_frmLst.wp_ePEODP_PLODP4_Fin = New moe_Estructuras.ePEODP_PLODP4
            ReDim wl_aePEODP_PLODP4(0)
            ReDim wg_aePEODP_PLODP4(0)
            wl_aePEODP_PLODP4(0) = New moe_Estructuras.ePEODP_PLODP4
            wg_aePEODP_PLODP4(0) = New moe_Estructuras.ePEODP_PLODP4

            wl_frmLst.wp_ePEODP_PLODP4_Ini = wl_ePEODP_PLODP4_Ini
            wl_frmLst.wp_ePEODP_PLODP4_Fin = wl_ePEODP_PLODP4_Fin
            wg_sTblCon1 = "PEODP_PLODP4_QIniFin"
            wl_frmLst.ShowDialog()
            If wg_aePEODP_PLODP4.Length <> 0 Then
                If wg_aePEODP_PLODP4(0).sCodDoc <> "" Then
                    wl_aePEODP_PLODP4 = wg_aePEODP_PLODP4
                End If
            End If
        Catch ex As Exception
            MsgBox("Error :  BD_frmPEODP_PLODP4_QGrl")
            MsgBox(ex.Message)
        End Try

    End Sub


#End Region
#End Region

#Region "050 CommandButtons_Click ABRECO Controladores de Eventos (Remitente_evento)"


#Region "CommandButtons_Click ABRECO Controladores de Eventos (Remitente_evento)"
    Private Sub cmdOk_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdOk.Click
        Try
            wp_sCmdSel = "O"
            Plo_VdaVar()
            BD_frmPEODP_PLODP4_QIniFin()
            If wl_aePEODP_PLODP4.Length <> 0 Then
                If wl_aePEODP_PLODP4(0).sCodDoc <> "" Then
                    wl_aePEODP_PLODP4 = wg_aePEODP_PLODP4
                    Rpt_PrmRpt()
                End If

            End If

        Catch ex As Exception
            MsgBox("Error : cmdOk_Click")
            MsgBox(ex.Message)
        End Try

    End Sub

    Private Sub cmdCancelar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdCancelar.Click
        Try
            grl_Cerrar()
        Catch ex As Exception
            MsgBox("Error : cmdCancelar_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub cmdLimpiar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdLimpiar.Click
        Try
            mof_fCnfCtrl(Me)
            mof_fLmpCtrl(Me)
            mof_IniciarVar()
            Plo_IniciarFrm()
            Plo_IniciarEst()
            Plo_IniciarVar()
            txtsCodDocIni.Focus()
        Catch ex As Exception
            MsgBox("Error : cmdLimpiar_Click")
            MsgBox(ex.Message)
        End Try
    End Sub

#End Region
#End Region

#Region "060 CommandButtons_Click Lista Controladores de Eventos (Remitente_evento)"
    Private Sub cmdABEOPFin_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdABEOPFin.Click
        Try
            wl_ePEODP_PLODP4 = wl_ePEODP_PLODP4_Fin
            wl_ePEODP_PLODP4.sEstOdp = " "
            vl_sCodStr = "EPEODP_QEODP"
            BD_frmPEODP_PLODP4_QGrl()
            If wl_ePEODP_PLODP4_QGrl.sEstOdp <> "" Then
                txtsEstOdpFin.Text = wl_ePEODP_PLODP4_QGrl.sEstOdp
                wl_sEstOdp = wl_ePEODP_PLODP4_QGrl.sEstOdp
                wl_ePEODP_PLODP4_Fin.sEstOdp = wl_ePEODP_PLODP4_QGrl.sEstOdp
                cmdLstPEODPFin.Focus()
            End If
        Catch ex As Exception
            MsgBox("Error : cmdABEOPFin_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub cmdABEOPIni_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdABEOPIni.Click
        Try
            wl_ePEODP_PLODP4 = wl_ePEODP_PLODP4_Ini
            wl_ePEODP_PLODP4.sEstOdp = " "
            vl_sCodStr = "EPEODP_QEODP"
            BD_frmPEODP_PLODP4_QGrl()
            If wl_ePEODP_PLODP4_QGrl.sEstOdp <> "" Then
                txtsEstOdpIni.Text = wl_ePEODP_PLODP4_QGrl.sEstOdp
                wl_sEstOdp = wl_ePEODP_PLODP4_QGrl.sEstOdp
                wl_ePEODP_PLODP4_Ini.sEstOdp = wl_ePEODP_PLODP4_QGrl.sEstOdp
                cmdLstPEODPFin.Focus()
            End If
        Catch ex As Exception
            MsgBox("Error : cmdABEOPIni_Click")
            MsgBox(ex.Message)
        End Try

    End Sub
    Private Sub cmdLstABODPFin_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdLstABODPFin.Click
        Try
            wl_ePEODP_PLODP4 = wl_ePEODP_PLODP4_Fin
            wl_ePEODP_PLODP4.sCodDoc = " "
            vl_sCodStr = "EPEODP_QCDOC"
            BD_frmPEODP_PLODP4_QGrl()
            If wl_ePEODP_PLODP4_QGrl.sCodDoc <> "" Then
                txtsCodDocFin.Text = wl_ePEODP_PLODP4_QGrl.sCodDoc
                wl_sCodDoc = wl_ePEODP_PLODP4_QGrl.sCodDoc
                wl_ePEODP_PLODP4_Fin.sCodDoc = wl_ePEODP_PLODP4_QGrl.sCodDoc
                cmdAPLND1Ini.Focus()
            End If
        Catch ex As Exception
            MsgBox("Error : cmdLstABODPFin_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub cmdLstABODPIni_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdLstABODPIni.Click
        Try
            Dim wl_frmLst As New frmGQLst
            vl_sCodStr = "EPEODP_QCDOC"
            wl_ePEODP_PLODP4 = wl_ePEODP_PLODP4_Ini
            wl_ePEODP_PLODP4.sCodDoc = " "
            BD_frmPEODP_PLODP4_QGrl()
            If wl_ePEODP_PLODP4_QGrl.sCodDoc <> "" Then
                txtsCodDocIni.Text = wl_ePEODP_PLODP4_QGrl.sCodDoc
                wl_sCodDoc = wl_ePEODP_PLODP4_QGrl.sCodDoc
                wl_ePEODP_PLODP4_Ini.sCodDoc = wl_ePEODP_PLODP4_QGrl.sCodDoc
                wl_ePEODP_PLODP4.sCodDoc = wl_ePEODP_PLODP4_QGrl.sCodDoc
                cmdLstABODPFin.Focus()
            End If
        Catch ex As Exception
            MsgBox("Error : cmdLstABODPIni_Click_1")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub cmdAPLND1Fin_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdAPLND1Fin.Click
        Try
            Dim wl_frmLst As New frmGQLst
            vl_sCodStr = "EPEODP_QCSER"
            wl_ePEODP_PLODP4 = wl_ePEODP_PLODP4_Fin
            wl_ePEODP_PLODP4.sCodSer = " "
            BD_frmPEODP_PLODP4_QGrl()
            If wl_ePEODP_PLODP4_QGrl.sCodSer <> " " Then
                txtsCodSerFin.Text = wl_ePEODP_PLODP4_QGrl.sCodSer
                wl_sCodSer = wl_ePEODP_PLODP4_QGrl.sCodSer
                wl_ePEODP_PLODP4_Fin.sCodSer = wl_ePEODP_PLODP4_QGrl.sCodSer
                cmdABEOPIni.Focus()
            End If
        Catch ex As Exception
            MsgBox("Error : cmdAPLND1Fin_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub cmdAPLND1Ini_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdAPLND1Ini.Click
        Try
            Dim wl_frmLst As New frmGQLst
            wl_ePEODP_PLODP4 = wl_ePEODP_PLODP4_Ini
            wl_ePEODP_PLODP4.sCodSer = " "
            vl_sCodStr = "EPEODP_QCSER"
            BD_frmPEODP_PLODP4_QGrl()
            If wl_ePEODP_PLODP4_QGrl.sCodSer <> "" Then
                txtsCodSerIni.Text = wl_ePEODP_PLODP4_QGrl.sCodSer
                wl_sCodSer = wl_ePEODP_PLODP4_QGrl.sCodSer
                wl_ePEODP_PLODP4_Ini.sCodSer = wl_ePEODP_PLODP4_QGrl.sCodSer
                wl_ePEODP_PLODP4.sCodSer = wl_ePEODP_PLODP4_QGrl.sCodSer
                cmdAPLND1Fin.Focus()
            End If
        Catch ex As Exception
            MsgBox("Error : cmdAPLND1Ini_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub cmdLstPEODPFin_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdLstPEODPFin.Click
        Try
            vl_sCodStr = "EPEODP_QNDOC"
            wl_ePEODP_PLODP4 = wl_ePEODP_PLODP4_Fin
            wl_ePEODP_PLODP4.nNroDoc = -1
            BD_frmPEODP_PLODP4_QGrl()
            If wl_ePEODP_PLODP4_QGrl.nNroDoc <> 0 Then
                txtnNroDocFin.Text = wl_ePEODP_PLODP4_QGrl.nNroDoc
                wl_ePEODP_PLODP4_Fin.nNroDoc = wl_ePEODP_PLODP4_QGrl.nNroDoc
                wl_nNroDoc = wl_ePEODP_PLODP4_QGrl.nNroDoc
                wl_ePEODP_PLODP4.nNroDoc = wl_ePEODP_PLODP4_QGrl.nNroDoc
                cmdOk.Focus()
            End If
        Catch ex As Exception
            MsgBox("Error : cmdLstPEODPFin_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub cmdLstPEODPIni_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdLstPEODPIni.Click
        Try
            vl_sCodStr = "EPEODP_QNDOC"
            wl_ePEODP_PLODP4 = wl_ePEODP_PLODP4_Ini
            wl_ePEODP_PLODP4.nNroDoc = -1

            BD_frmPEODP_PLODP4_QGrl()
            If wl_ePEODP_PLODP4_QGrl.nNroDoc <> 0 Then
                txtnNroDocIni.Text = wl_ePEODP_PLODP4_QGrl.nNroDoc
                wl_ePEODP_PLODP4_Ini.nNroDoc = wl_ePEODP_PLODP4_QGrl.nNroDoc
                wl_nNroDoc = wl_ePEODP_PLODP4_QGrl.nNroDoc
                wl_ePEODP_PLODP4.nNroDoc = wl_ePEODP_PLODP4_QGrl.nNroDoc
                cmdLstPEODPFin.Focus()
            End If
        Catch ex As Exception
            MsgBox("Error : cmdLstPEODPIni_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub cmdPEODP_QFEmiFin_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPEODP_QFEmiFin.Click
        Try
            wl_ePEODP_PLODP4 = wl_ePEODP_PLODP4_Fin
            wl_ePEODP_PLODP4.dFecEmi = dtpFecEmiFin.Text
            vl_sCodStr = "EPEODP_QFEMI"

            BD_frmPEODP_PLODP4_QGrl()
            If wl_ePEODP_PLODP4_QGrl.dFecEmi <> #1/1/1901# Then
                '  txtdFecEmiFin.Text = wl_ePEODP_PLODP4_QGrl.dFecEmi
                wl_ePEODP_PLODP4_Fin.dFecEmi = wl_ePEODP_PLODP4_QGrl.dFecEmi
                wl_ePEODP_PLODP4.dFecEmi = wl_ePEODP_PLODP4_QGrl.dFecEmi
                cmdOk.Focus()
            End If
        Catch ex As Exception
            MsgBox("Error : cmdPEODP_QFEmiFin_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub cmdPEODP_QFEmiIni_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPEODP_QFEmiIni.Click

        Try
            wl_ePEODP_PLODP4 = wl_ePEODP_PLODP4_Ini
            wl_ePEODP_PLODP4.dFecEmi = dtpFecEmiIni.Text
            vl_sCodStr = "EPEODP_QFEMI"

            BD_frmPEODP_PLODP4_QGrl()
            If wl_ePEODP_PLODP4_QGrl.dFecEmi <> #1/1/1901# Then
                ' txtdFecEmiIni.Text = wl_ePEODP_PLODP4_QGrl.dFecEmi
                wl_ePEODP_PLODP4_Ini.dFecEmi = wl_ePEODP_PLODP4_QGrl.dFecEmi
                wl_ePEODP_PLODP4.dFecEmi = wl_ePEODP_PLODP4_QGrl.dFecEmi
                cmdOk.Focus()
            End If
        Catch ex As Exception
            MsgBox("Error : cmdPEODP_QFEmiIni_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub cmdPEODP_QFFinFin_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPEODP_QFFinFin.Click
        Try
            wl_ePEODP_PLODP4 = wl_ePEODP_PLODP4_Fin
            wl_ePEODP_PLODP4.dFecFin = #1/1/1901#
            vl_sCodStr = "EPEODP_QFFIN"

            BD_frmPEODP_PLODP4_QGrl()
            If wl_ePEODP_PLODP4_QGrl.dFecFin <> #1/1/1901# Then
                txtdFecFinFin.Text = wl_ePEODP_PLODP4_QGrl.dFecFin
                wl_ePEODP_PLODP4_Fin.dFecFin = wl_ePEODP_PLODP4_QGrl.dFecFin
                wl_ePEODP_PLODP4.dFecFin = wl_ePEODP_PLODP4_QGrl.dFecFin
                cmdOk.Focus()
            End If
        Catch ex As Exception
            MsgBox("Error : cmdPEODP_QFFinFin_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub cmdPEODP_QFFinIni_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPEODP_QFFinIni.Click
        Try
            wl_ePEODP_PLODP4 = wl_ePEODP_PLODP4_Ini
            wl_ePEODP_PLODP4.dFecFin = #1/1/1901#
            vl_sCodStr = "EPEODP_QFFIN"

            BD_frmPEODP_PLODP4_QGrl()
            If wl_ePEODP_PLODP4_QGrl.dFecFin <> #1/1/1901# Then
                txtdFecFinIni.Text = wl_ePEODP_PLODP4_QGrl.dFecFin
                wl_ePEODP_PLODP4_Fin.dFecFin = wl_ePEODP_PLODP4_QGrl.dFecFin
                wl_ePEODP_PLODP4.dFecFin = wl_ePEODP_PLODP4_QGrl.dFecFin
                cmdOk.Focus()
            End If
        Catch ex As Exception
            MsgBox("Error : cmdPEODP_QFFinIni_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub cmdPEODP_QFIniFin_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPEODP_QFIniFin.Click
        Try
            wl_ePEODP_PLODP4 = wl_ePEODP_PLODP4_Fin
            wl_ePEODP_PLODP4.dFecIni = #1/1/1901#
            vl_sCodStr = "EPEODP_QFINI"

            BD_frmPEODP_PLODP4_QGrl()
            If wl_ePEODP_PLODP4_QGrl.dFecIni <> #1/1/1901# Then
                txtdFecIniFin.Text = wl_ePEODP_PLODP4_QGrl.dFecIni
                wl_ePEODP_PLODP4_Fin.dFecIni = wl_ePEODP_PLODP4_QGrl.dFecIni
                wl_ePEODP_PLODP4.dFecIni = wl_ePEODP_PLODP4_QGrl.dFecIni
                cmdOk.Focus()
            End If
        Catch ex As Exception
            MsgBox("Error : cmdPEODP_QFIniFin_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub cmdPEODP_QFIniIni_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPEODP_QFIniIni.Click
        Try
            wl_ePEODP_PLODP4 = wl_ePEODP_PLODP4_Ini
            wl_ePEODP_PLODP4.dFecIni = #1/1/1901#
            vl_sCodStr = "EPEODP_QFINI"

            BD_frmPEODP_PLODP4_QGrl()
            If wl_ePEODP_PLODP4_QGrl.dFecIni <> #1/1/1901# Then
                txtdFecIniIni.Text = wl_ePEODP_PLODP4_QGrl.dFecIni
                wl_ePEODP_PLODP4_Fin.dFecIni = wl_ePEODP_PLODP4_QGrl.dFecIni
                wl_ePEODP_PLODP4.dFecIni = wl_ePEODP_PLODP4_QGrl.dFecIni
                cmdOk.Focus()
            End If
        Catch ex As Exception
            MsgBox("Error : cmdPEODP_QFIniIni_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub cmdPEODP_QItmFin_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPEODP_QItmFin.Click
        Try

            wl_ePEODP_PLODP4 = wl_ePEODP_PLODP4_Fin
            wl_ePEODP_PLODP4.sCodItm = " "
            vl_sCodStr = "EPEODP_QITM"
            BD_frmPEODP_PLODP4_QGrl()
            If wl_ePEODP_PLODP4_QGrl.sCodItm <> "" Then
                txtsCodItmFin.Text = wl_ePEODP_PLODP4_QGrl.sCodItm
                wl_sCodItm = wl_ePEODP_PLODP4_QGrl.sCodItm
                wl_ePEODP_PLODP4_Fin.sCodItm = wl_ePEODP_PLODP4_QGrl.sCodItm
                wl_ePEODP_PLODP4.sCodItm = wl_ePEODP_PLODP4_QGrl.sCodItm
                cmdOk.Focus()
            End If
        Catch ex As Exception
            MsgBox("Error : cmdPEODP_QItmFin_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub cmdPEODP_QItmIni_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPEODP_QItmIni.Click
        Try
            wl_ePEODP_PLODP4 = wl_ePEODP_PLODP4_Ini
            wl_ePEODP_PLODP4.sCodItm = " "
            vl_sCodStr = "EPEODP_QITM"
            BD_frmPEODP_PLODP4_QGrl()
            If wl_ePEODP_PLODP4_QGrl.sCodItm <> "" Then
                txtsCodItmIni.Text = wl_ePEODP_PLODP4_QGrl.sCodItm
                wl_sCodItm = wl_ePEODP_PLODP4_QGrl.sCodItm
                wl_ePEODP_PLODP4_Ini.sCodItm = wl_ePEODP_PLODP4_QGrl.sCodItm
                wl_ePEODP_PLODP4.sCodItm = wl_ePEODP_PLODP4_QGrl.sCodItm
                cmdPEODP_QItmFin.Focus()
            End If
        Catch ex As Exception
            MsgBox("Error : cmdPEODP_QItmIni_Click")
            MsgBox(ex.Message)
        End Try
    End Sub
#End Region

#Region "080 Plo "
#Region "080 Plo Procedimientos del Formulario. Personalizados"
    Private Sub Plo_IniciarEst()
        Try
            ReDim wl_aeABEOP(0)
            ReDim wl_aeABODP(0)
            ReDim wl_aeAPLND1(0)
            ReDim wl_aePEODP_PLODP4(0)
            ReDim wl_aePEODP_PLODP4_Fin(0)
            ReDim wl_aePEODP_PLODP4_QGrl(0)
            ReDim wl_aePEODP_PLODP4_Ini(0)
            wl_eABEOP = New moe_Estructuras.eABEOP
            wl_eABODP = New moe_Estructuras.eABODP
            wl_eAPLND1 = New moe_Estructuras.eAPLND1
            wl_ePEODP_PLODP4 = New moe_Estructuras.ePEODP_PLODP4
            wl_ePEODP_PLODP4_Ini = New moe_Estructuras.ePEODP_PLODP4
            wl_ePEODP_PLODP4_QGrl = New moe_Estructuras.ePEODP_PLODP4
            wl_ePEODP_PLODP4_Fin = New moe_Estructuras.ePEODP_PLODP4
        Catch ex As Exception
            MsgBox("Error : Plo_IniciarEst")
            MsgBox(ex.Message)
        End Try

    End Sub
    Private Sub Plo_IniciarFrm()
        Try
            wl_sNomFrm = "frmRPODP01"
            dtpFecEmiIni.Format = DateTimePickerFormat.Custom
            dtpFecEmiIni.CustomFormat = "yyyy-MM-dd"
            dtpFecEmiFin.Format = DateTimePickerFormat.Custom
            dtpFecEmiFin.CustomFormat = "yyyy-MM-dd"



        Catch ex As Exception
            MsgBox("Error : Plo_IniciaFrm")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub Plo_IniciarVar()
        Try

            wl_bYesNo = False

            wl_sCodErrAdi = "10-319"
            wl_sCodErrCon = "10-119"
            wl_sCodErrOk = "20-001"
            '       wl_sDesMsgEli = "Este opción elimina el código. ¿ Continua ?"
            '       wl_sTitMsgEli = "Mensaje del sistema"

            ' globales
            wl_sCmpTSB = "sCodItm"
            wl_sDatTSB = ""
            wl_sCodDoc = ""
            wl_sCodSer = ""
            wl_nNroDoc = 0
            wl_sEstOdp = ""
            wl_sCodItm = ""
            vl_sCodStr = ""
            dtpFecEmiFin.Text = Today
            dtpFecEmiIni.Text = Year(Today).ToString + "-01-01"


        Catch ex As Exception
            MsgBox("Error : Plo_IniciarVar")
            MsgBox(ex.Message)
        End Try

    End Sub

    Private Sub Plo_VdaVar()
        Try

            If txtsCodDocFin.Text = "" Then
                txtsCodDocFin.Text = txtsCodDocIni.Text
            End If
            wl_ePEODP_PLODP4_Fin.sCodDoc = txtsCodDocFin.Text

            If txtsCodDocIni.Text = "" Then
                txtsCodDocIni.Text = txtsCodDocFin.Text
            End If
            wl_ePEODP_PLODP4_Ini.sCodDoc = txtsCodDocIni.Text

            If txtsCodSerFin.Text = "" Then
                txtsCodSerFin.Text = txtsCodSerIni.Text
            End If
            wl_ePEODP_PLODP4_Fin.sCodSer = txtsCodSerFin.Text

            If txtsCodSerIni.Text = "" Then
                txtsCodSerIni.Text = txtsCodSerFin.Text
            End If
            wl_ePEODP_PLODP4_Ini.sCodSer = txtsCodSerIni.Text

            If txtnNroDocFin.Text = "" Then
                If txtnNroDocIni.Text = "" Then
                    txtnNroDocIni.Text = "0"
                End If
                txtnNroDocFin.Text = txtnNroDocIni.Text
            End If
            wl_ePEODP_PLODP4_Fin.nNroDoc = CDbl(txtnNroDocFin.Text)
            If txtnNroDocIni.Text = "" Then
                txtnNroDocIni.Text = txtnNroDocFin.Text
            End If
            wl_ePEODP_PLODP4_Ini.nNroDoc = CDbl(txtnNroDocIni.Text)
            If txtsEstOdpFin.Text = "" Then
                txtsEstOdpFin.Text = txtsEstOdpIni.Text
            End If
            wl_ePEODP_PLODP4_Fin.sEstOdp = txtsEstOdpFin.Text
            If txtsEstOdpIni.Text = "" Then
                txtsEstOdpIni.Text = txtsEstOdpFin.Text
            End If
            wl_ePEODP_PLODP4_Ini.sEstOdp = txtsEstOdpIni.Text

            If txtsCodItmFin.Text = "" Then
                txtsCodItmFin.Text = txtsCodItmIni.Text
            End If
            wl_ePEODP_PLODP4_Fin.sCodItm = txtsCodItmFin.Text

            If txtsCodItmIni.Text = "" Then
                txtsCodItmIni.Text = txtsCodItmFin.Text
            End If
            wl_ePEODP_PLODP4_Ini.sCodItm = txtsCodItmIni.Text

            If dtpFecEmiFin.Text = "" Then
                dtpFecEmiFin.Text = Today
            End If
            wl_ePEODP_PLODP4_Fin.dFecEmi = dtpFecEmiFin.Text

            If dtpFecEmiIni.Text = "" Then
                dtpFecEmiIni.Text = Year(Today).ToString + "-01-01"
            End If
            wl_ePEODP_PLODP4_Ini.dFecEmi = dtpFecEmiIni.Text


            If txtdFecIniFin.Text = "" Then
                If txtdFecIniIni.Text = "" Then
                    txtdFecIniIni.Text = #1/1/1901#
                    'txtdFecIniIni.Text = Today
                End If
                txtdFecIniFin.Text = txtdFecIniIni.Text
            End If
            wl_ePEODP_PLODP4_Fin.dFecIni = CDate(txtdFecIniFin.Text)
            If txtdFecIniFin.Text = #1/1/1901# Then
                txtdFecIniFin.Text = ""
            End If
            If txtdFecIniIni.Text = "" Then
                If txtdFecIniFin.Text = "" Then
                    txtdFecIniFin.Text = #1/1/1901#
                    'txtdFecIniFin.Text = Today
                End If
                txtdFecIniIni.Text = txtdFecIniFin.Text
            End If
            wl_ePEODP_PLODP4_Ini.dFecIni = CDate(txtdFecIniIni.Text)
            If txtdFecIniIni.Text = #1/1/1901# Then
                txtdFecIniIni.Text = ""
            End If

            If txtdFecFinFin.Text = "" Then
                If txtdFecFinIni.Text = "" Then
                    txtdFecFinIni.Text = #1/1/1901#
                    'txtdFecFinIni.Text = Today
                End If
                txtdFecFinFin.Text = txtdFecFinIni.Text
            End If
            wl_ePEODP_PLODP4_Fin.dFecFin = CDate(txtdFecFinFin.Text)
            If txtdFecFinFin.Text = #1/1/1901# Then
                txtdFecFinFin.Text = ""
            End If

            If txtdFecFinIni.Text = "" Then
                If txtdFecFinFin.Text = "" Then
                    txtdFecFinFin.Text = #1/1/1901#
                    'txtdFecFinFin.Text = Today
                End If
                txtdFecFinIni.Text = txtdFecFinFin.Text
            End If
            wl_ePEODP_PLODP4_Ini.dFecFin = CDate(txtdFecFinIni.Text)
            If txtdFecFinIni.Text = #1/1/1901# Then
                txtdFecFinIni.Text = ""
            End If
        Catch ex As Exception
            MsgBox("Error : Plo_VdaVar")
            MsgBox(ex.Message)
        End Try
    End Sub

#End Region
#Region "081 Plo Procedimientos del Formulario. Comunes"

#End Region
#Region "082 Elo Eventos locales del Formulario"
    Private Sub txtnNroDocFin_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            If txtnNroDocFin.Text = "" Then
                txtnNroDocFin.Text = "0"
            End If
            wl_ePEODP_PLODP4_Fin.nNroDoc = CDbl(txtnNroDocFin.Text)
        Catch ex As Exception
            MsgBox("Error : txtnNroDocFin_LostFocus")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub txtnNroDocIni_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            If txtnNroDocIni.Text = "" Then
                txtnNroDocIni.Text = "0"
            End If
            wl_ePEODP_PLODP4_Ini.nNroDoc = CDbl(txtnNroDocIni.Text)
        Catch ex As Exception
            MsgBox("Error : txtnNroDocIni_LostFocus")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub txtsCodDocFin_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            wl_ePEODP_PLODP4_Fin.sCodDoc = txtsCodDocFin.Text
        Catch ex As Exception
            MsgBox("Error : txtsCodDocFin_LostFocus")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub txtsCodDocIni_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            wl_ePEODP_PLODP4_Ini.sCodDoc = txtsCodDocIni.Text
        Catch ex As Exception
            MsgBox("Error : txtsCodDocIni_LostFocus")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub txtsCodSerFin_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            wl_ePEODP_PLODP4_Fin.sCodSer = txtsCodSerFin.Text
        Catch ex As Exception
            MsgBox("Error : txtsCodSerFin_LostFocus")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub txtsCodSerIni_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            wl_ePEODP_PLODP4_Ini.sCodSer = txtsCodSerIni.Text
        Catch ex As Exception
            MsgBox("Error : txtsCodSerIni_LostFocus")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub txtsEstOdpFin_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            wl_ePEODP_PLODP4_Fin.sEstOdp = txtsEstOdpFin.Text
        Catch ex As Exception
            MsgBox("Error : txtsEstOdpFin_LostFocus")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub txtsEstOdpIni_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            wl_ePEODP_PLODP4_Ini.sEstOdp = txtsEstOdpIni.Text
        Catch ex As Exception
            MsgBox("Error : txtsEstOdpIni_LostFocus")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub txtsCodItmFin_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            wl_ePEODP_PLODP4_Fin.sCodItm = txtsCodItmFin.Text
        Catch ex As Exception
            MsgBox("Error : txtsCodItmFin_LostFocus")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub txtsCodItmIni_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            wl_ePEODP_PLODP4_Ini.sCodItm = txtsCodItmIni.Text
        Catch ex As Exception
            MsgBox("Error : txtsCodItmIni_LostFocus")
            MsgBox(ex.Message)
        End Try
    End Sub

    Private Sub txtdFecFinFin_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            If txtdFecFinFin.Text = "" Then
                txtdFecFinFin.Text = #1/1/1901#
            End If
            wl_ePEODP_PLODP4_Fin.dFecFin = CDate(txtdFecFinFin.Text)
        Catch ex As Exception
            MsgBox("Error : txtdFetxtdFecFinFin_LostFocuscEmiFin_LostFocus")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub txtdFecFinIni_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            If txtdFecFinIni.Text = "" Then
                txtdFecFinIni.Text = #1/1/1901#
            End If
            wl_ePEODP_PLODP4_Ini.dFecFin = CDate(txtdFecFinIni.Text)
        Catch ex As Exception
            MsgBox("Error : txtdFecFinIni_LostFocus")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub txtdFecIniFin_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            If txtdFecIniFin.Text = "" Then
                txtdFecIniFin.Text = #1/1/1901#
            End If
            wl_ePEODP_PLODP4_Fin.dFecIni = CDate(txtdFecIniFin.Text)
        Catch ex As Exception
            MsgBox("Error : txtdFecIniFin_LostFocus")
            MsgBox(ex.Message)
        End Try
    End Sub
    Private Sub txtdFecIniIni_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            If txtdFecIniIni.Text = "" Then
                txtdFecIniIni.Text = #1/1/1901#
            End If
            wl_ePEODP_PLODP4_Ini.dFecIni = CDate(txtdFecIniIni.Text)
        Catch ex As Exception
            MsgBox("Error : txtdFecIniIni_LostFocus")
            MsgBox(ex.Message)
        End Try
    End Sub
#End Region
#End Region

#Region "090 Rpt - Reportes"
    Private Sub Rpt_PrmRpt()
        Try
            Dim wl_bSDM_EDM As Boolean
            wl_bSDM_EDM = False

            ReDim wl_aeABEOP(0)
            wl_aeABEOP(0) = New moe_Estructuras.eABEOP
            wl_DTL2 = New DataTable
            wl_bEOF = False
            wl_DTL2 = clnTblAB.cndtCrgABEOP(wl_bEOF)
            moo_aeABEOP_DT(wl_aeABEOP,
                           wl_DTL2)

            wl_sNroALetras = ""

            If wl_aePEODP_PLODP4(0).sCodDoc <> "" Then
                Dim wl_frmRPT01 As New rptPRODP05
                wl_frmRPT01.wp_aePEODP_PLODP4 = wl_aePEODP_PLODP4
                wl_frmRPT01.Show()
            End If

        Catch ex As Exception
            MsgBox("Error :  Rpt_PrmRpt")
            MsgBox(ex.Message)
        End Try
    End Sub
#End Region

#Region "150 grl Otros"
    Private Sub grl_Cerrar()
        mof_inhabTsbBuscar()
        mof_inhabTsbCrear()
        mof_inhabTsbPASU()
        mof_inhabTsbEliminar()
        mof_LmpMsgError()
        Me.Close()
        mof_LmpMsgError()
    End Sub

#End Region

End Class
