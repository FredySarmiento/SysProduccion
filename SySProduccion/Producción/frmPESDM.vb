'Tipo de formulario: Maestro detalle: GEMOV - GLEMOV1 : PESDM. Salida de materiales para producción
Public Class frmPESDM
...
#Region "020 frm Eventos - Principal"
    Private Sub frmPESDM_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Disposed
        Try
            grl_RemoveHandler()
            Me.Dispose()
        Catch ex As Exception
            MsgBox("Error :  frmPESDM_Disposed")
            MsgBox(ex.Message)
        End Try

    End Sub

    Private Sub frmPESDM_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            Me.Text += "  -  " + vg_sNomCia
            grl_RemoveHandler()
            grl_AddHandler()
    ...
    Private Sub BD_Var_eGEMOV()
        Try
            With wl_eGEMOV
                ...
                vl_bPrp03 = .bPrp03
            End With
            If vl_sCodAut = "00" Or vl_sCodAut = "" Then
                Plo_ActivarCmpAut()
            End If
...
#Region "042 BD Busquedas"
    ...
    Private Function BD_BuscarIELDMI1() As Boolean
        Try
            ...
            wr_bEOFE2 = mof_fConIELDMI1(vl_sCodItm,
                                        wl_eIELDMI1)
        Catch ex As Exception
            MsgBox(ex.Message)
            MsgBox("Error en el SUB: BD_BuscarIELDMI1")
        End Try
        Return wr_bEOFE2
    End Function
...
#Region "043 BD Cargar"
    ...
    Private Sub BD_CargarIEDMIR()
        ...
        End Try
    End Sub
    Private Sub BD_CargarIELDMI1(ByVal wl_CellCod As Integer,
        ...
        Try
            ...
            wg_sTblCon1 = "IELDMI1"
            wl_frmLst.wp_sFrmOri = "frmPESDM"
            wl_frmLst.ShowDialog()
            wl_nRowAct = wl_CtrlDgv.CurrentCellAddress.Y
            If wl_frmLst.wp_eIELDMI1.eIEDMI.sCodItm <> "" Then
                wl_eIELDMI1 = wl_frmLst.wp_eIELDMI1
...
#Region "060 CommandButtons_Click Lista Controladores de Eventos (Remitente_evento)"
    ...
    End Sub
    Private Sub cmdLstPESDM_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdLstPESDM.Click
        Try
            Dim wl_frmLst As New frmGQLst
            wg_sTblCon1 = "PESDM"
            wl_eGEMOV = New moe_Estructuras.eGEMOV
    ...
    Private Sub cmdPEODP_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPEODP.Click
        Try
            ...
            wl_frmLst.wp_sTxtFrm = "Consulta ordenes de producción"
            wl_frmLst.ShowDialog()
            wl_ePEODP = wl_frmLst.wp_ePEODP
            wg_DTCon1 = New DataTable
            If wl_ePEODP.sCodDoc <> "" Then
                Plo_frmLstdet()
                If wg_aePLODP1(0).sCodDoc <> "" Then
                    dgv_LstADgv()
                End If
        ...
        Try
            ...
            With wl_CtrlDgv
                For wl_nRow = 0 To wl_nRowDgv
                    ...
                    '                    If .Rows(wl_nRow).Cells("bIndCan").Value = True Then
                    ...
                    ' Else
                    ' wl_CtrlDgv.Rows(wl_nRow).DefaultCellStyle.ForeColor = Color.Black
                    ' End If
                    ' End If
                    ' If wl_CtrlDgv.Rows(wl_nRow).DefaultCellStyle.ForeColor = Color.Red Then
                    ' If .Rows(wl_nRow).Cells("bIndCos").Value = True Then
                    ' If .Rows(wl_nRow).Cells("nPreUni").Value <= 0 Then
                    ' wl_CtrlDgv.Rows(wl_nRow).DefaultCellStyle.ForeColor = Color.Red
                    ' Else
                    ' wl_CtrlDgv.Rows(wl_nRow).DefaultCellStyle.ForeColor = Color.Black
                    ' End If
...
#Region "080 Plo Procedimientos del Formulario. Personalizados"
    ...
    Private Sub Plo_frmLstdet()
        Try
            ...
            ReDim wg_aePLODP1(0)
            wg_aePLODP1(0) = New moe_Estructuras.ePLODP1
            wl_frmLstDet.wp_sNomFrm = "Consulta detalle de la orden de producción"
            ReDim wl_aePLODP1(0)
            wl_frmLstDet.ShowDialog()
            If wg_aePLODP1(0).sCodDoc <> "" Then
                wl_aePLODP1 = wg_aePLODP1
                dgv_LstADgv()
            End If
...
#Region "081 Plo Procedimientos del Formulario. Comunes"
    Private Sub Plo_ActivarCmpAct()
        Try
            ...
            cmdPEODP.Enabled = True
            cmdLstPESDM.Enabled = False
        Catch ex As Exception
